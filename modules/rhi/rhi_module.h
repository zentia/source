...
namespace source_runtime
{
	...
	public:
		...
		virtual void create_buffer(rhi_device_size size, rhi_buffer_usage_flags usage, rhi_memory_property_flags properties, rhi_buffer*& buffer, rhi_device_memory*& buffer_memory, void* data = nullptr, int data_size = 0) = 0;
		...
		virtual bool create_buffer_with_alignment_vma(VmaAllocator allocator, 
			const rhi_buffer_create_info* buffer_create_info, VmaAllocationCreateInfo* allocation_create_info, rhi_device_size min_alignment, rhi_buffer*& buffer, VmaAllocation* allocation, VmaAllocationInfo* allocation_info) = 0;
		...
		virtual void create_image(uint32_t image_width, uint32_t image_height, rhi_format format,
		...
		virtual bool create_frame_buffer(const rhi_frame_buffer_create_info* create_info, rhi_frame_buffer*& frame_buffer) = 0;
		virtual bool create_graphics_pipelines(rhi_pipeline_cache* pipeline_cache, uint32_t create_info_count, const rhi_graphics_pipeline_create_info* create_info, rhi_pipeline*& pipeline) = 0;
		...
		virtual bool wait_for_fences(uint32_t fence_count, rhi_fence* const* fence, rhi_bool32 wait_all, uint64_t timeout) = 0;
		virtual bool reset_fences(uint32_t fence_count, rhi_fence* const* fence) = 0;
		virtual bool reset_command_pool(rhi_command_pool* command_pool, rhi_command_pool_reset_flags flags) = 0;
		virtual bool begin_command_buffer(rhi_command_buffer* command_buffer, const rhi_command_buffer_begin_info* begin_info) = 0;
		virtual bool end_command_buffer(rhi_command_buffer* command_buffer) = 0;
		virtual void cmd_begin_render_pass(rhi_command_buffer* command_buffer, const rhi_render_pass_begin_info* render_pass_begin, rhi_sub_pass_contents contents) = 0;
		virtual void cmd_next_sub_pass(rhi_command_buffer* command_buffer, rhi_sub_pass_contents contents) = 0;
		virtual void cmd_end_render_pass(rhi_command_buffer* command_buffer) = 0;
		virtual void cmd_bind_pipeline(rhi_command_buffer* command_buffer, rhi_pipeline_bind_point pipeline_bind_point, rhi_pipeline* pipeline) = 0;
		virtual void cmd_set_viewport(rhi_command_buffer* command_buffer, uint32_t first_viewport, uint32_t viewport_count, const rhi_viewport* viewports) = 0;
		virtual void cmd_set_scissor(rhi_command_buffer* command_buffer, uint32_t first_scissor, uint32_t scissor_count, const rhi_rect_2d* scissors) = 0;
		virtual void cmd_bind_vertex_buffers(rhi_command_buffer* command_buffer, uint32_t first_binding, uint32_t binding_count, rhi_buffer* const* buffers, const rhi_device_size* offsets) = 0;
		virtual void cmd_bind_index_buffer(rhi_command_buffer* command_buffer, rhi_buffer* buffer, rhi_device_size offset, rhi_index_type index_type) = 0;
		...
		virtual void cmd_draw_indexed(rhi_command_buffer* command_buffer, uint32_t index_count, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance) = 0;
		virtual void cmd_clear_attachments(rhi_command_buffer* command_buffer, uint32_t attachment_count, const rhi_clear_attachment* attachment, uint32_t rect_count, const rhi_clear_rect* rects) = 0;

		// query
		virtual void get_physical_device_properties(rhi_physical_device_properties* properties) = 0;
		virtual [[nodiscard]] rhi_command_buffer* get_current_command_buffer() const = 0;
		virtual rhi_command_buffer* const* get_command_buffer_list() const = 0;
		virtual rhi_command_pool* get_command_pool() const = 0;
		virtual rhi_descriptor_pool* get_descriptor_pool() const = 0;
		virtual rhi_fence* const* get_fence_list() const = 0;
		virtual queue_family_indices get_queue_family_indices() const = 0;
		virtual rhi_queue* get_graphics_queue() const = 0;
		virtual rhi_queue* get_compute_queue() const = 0;
		virtual rhi_swap_chain_desc get_swap_chain_info() = 0;
		virtual rhi_depth_image_desc get_depth_image_info() = 0;
