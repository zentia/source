...
#   define PLATFORM_POSIX_SOCKET_IP6_SUPPORT 1
#endif
...
// Put internal classes under inline namespace when active to avoid potentially duplicated symbols.
    BASELIB_CPP_INTERFACE
    {
            ...
            socketaddr(const Baselib_NetworkAddress* address, Baselib_ErrorState* errorState)
            {
...
#if PLATFORM_POSIX_SOCKET_IP6_SUPPORT
                ...
                switch (address->family)
                {
                    ...
                    case Baselib_NetworkAddress_Family_IPv6:
                        ...
                        in6()->sin6_scope_id = address->ipv6_scope_id;
                        break;
#endif
                    default:
            ...
            void toBaselib(Baselib_NetworkAddress* dst, Baselib_ErrorState* errorState) const
            {
                ...
                switch (addr()->sa_family)
                {
                    ...
                    case AF_INET6:
                        ...
                        dst->ipv6_scope_id = in6()->sin6_scope_id;
                        break;
#endif
                    default:
        ...
        static Baselib_ErrorCode SocketErrorToErrorCode(int error)
        {
            switch (error)
            {
                ...
                case EHOSTUNREACH:
                case ETIMEDOUT:
                case ENODEV: // happens when specifying incorrect ipv6 scope id
        ...
        static void SetNonblocking(int socketFd, Baselib_ErrorState* errorState)
        {
            ...
            if ((currentFdFlags == -1) || (::fcntl(socketFd, F_SETFL, currentFdFlags | O_NONBLOCK) == -1))
            {
                ...
                errorState |= RaiseError(Baselib_ErrorCode_UnexpectedError) | WithErrno(error);
                ::close(socketFd);
            }
        }
        ...
        static void SetReuseaddr(Baselib_Socket_Handle socket, Baselib_ErrorState* errorState)
        {
            ...
            {
                // If ENOPROTOOPT is raised we ignore the error.
                //
                // Even though the constant is declared the platform may not suppport SO_REUSEPORT.
                // Which is fine, it's an optional feature.
                const int error = errno;
        ...
        Baselib_ErrorState*    errorState)
    {
        ...
        for (size_t i = 0; i < socketsCount; i++)
        {
            ...
            else if (pollFds[i].revents & POLLHUP)
            {
                int socketErr;
                socklen_t len = sizeof(socketErr);
                if (::getsockopt(pollFds[i].fd, SOL_SOCKET, SO_ERROR, (void*)&socketErr, &len) == 0 && socketErr != 0)
                {
                    sockets[i].errorState |= RaiseError(detail::SocketErrorToErrorCode(socketErr)) | WithErrno(socketErr);
                }
            }
    ...
    BASELIB_INLINE_IMPL uint32_t Baselib_Socket_UDP_Recv(Baselib_Socket_Handle socket, Baselib_Socket_Message* messages, uint32_t messagesCount, Baselib_ErrorState* errorState)
    {
        ...
        for (uint32_t i = 0; i < messagesCount; ++i)
        {
            ...
            // On some platforms, srcAddress is always written but srcAddressSize will be set to 0
            if (message.address && (srcAddress.family() != 0 && srcAddressSize > 0))
            {
