    ...
    BASELIB_INLINE_IMPL uint32_t Baselib_Socket_UDP_Recv(Baselib_Socket_Handle socket, Baselib_Socket_Message* messages, uint32_t messagesCount, Baselib_ErrorState* errorState)
    {
        for (uint32_t i = 0; i < messagesCount; ++i)
        {
            ...
            if (bytesTransferred == SOCKET_ERROR)
            {
                ...
                // WSAEWOULDBLOCK just means there's nothing more.
                // WSAEINVAL means socket is not bound, but other platforms treat this as WSAEWOULDBLOCK.
                // WSAECONNRESET means a previous send resulted in an ICMP Port Unreachable message. Other
                // platforms don't error out in this case, and an error is actually detrimental if the
                // application already has a retry mechanism implemented.
                if (error != WSAEWOULDBLOCK && error != WSAEINVAL && error != WSAECONNRESET)
                    errorState |= RaiseError(detail::SockErrorToErrorCode(error)) | WithGetLastError(error);
