...
// Put internal classes under inline namespace when active to avoid potentially duplicated symbols.
    BASELIB_CPP_INTERFACE
    {
        ...
        private:
            ...
            RIOSockets()
            {
                ...
                SOCKET socket = ::WSASocketW(AF_INET6, SOCK_DGRAM, IPPROTO_UDP, NULL, 0, WSA_FLAG_REGISTERED_IO);
                if (socket == INVALID_SOCKET)
                    return;

                DWORD nBytes = 0;
                GUID functionTableId = WSAID_MULTIPLE_RIO;
                const int rioFunctionTableLookup = ::WSAIoctl(socket, SIO_GET_MULTIPLE_EXTENSION_FUNCTION_POINTER, &functionTableId, sizeof(GUID),
                    (void**)(&rioFunctionTable), sizeof(rioFunctionTable), &nBytes, NULL, NULL);

                // That the function table is available is not enough to guarantee that RIO will work.
                // TDI filters are known to cause issues with RIO and this will manifest as an error
                // when creating a request queue for a socket. See KERNEL-248 for details.
                isRioAvailable = (rioFunctionTableLookup == ERROR_SUCCESS && CanCreateRequestQueue(socket));

                const int socketClose = ::closesocket(socket);
                BaselibAssert(socketClose == ERROR_SUCCESS);
            }

            bool CanCreateRequestQueue(SOCKET socket)
            {
                RIO_CQ cq = rioFunctionTable.RIOCreateCompletionQueue(1, nullptr);
                if (cq == RIO_INVALID_CQ)
                    return false;

                // Values selected as to allocate as little memory as possible in the OS.
                RIO_RQ rq = rioFunctionTable.RIOCreateRequestQueue(socket, 1, 1, 0, 1, cq, cq, nullptr);
                rioFunctionTable.RIOCloseCompletionQueue(cq);

                return rq != RIO_INVALID_RQ;
            }
        };
            ...
            struct CompletionQueue
            {
                bool Init(uint32_t queueSize, Baselib_ErrorState* errorState)
                {
                    ...
                    if (completionEvent == WSA_INVALID_EVENT)
                    {
                        ...
                        const DWORD lastWSAError = WSAGetLastError();
                ...
                bool WaitForResult(uint32_t timeoutInMilliseconds, Baselib_ErrorState* errorState)
                {
                    ...
                    const DWORD errorCode = RIOSockets::GetFunctionTable().RIONotify(cq);
    ...
    BASELIB_INLINE_IMPL bool Baselib_RegisteredNetwork_IsEmulated()
    {
        return false;
    }
    ...
    )
    {
        ...
        Baselib_Internal_Memory_Free(socket);
    }
}
